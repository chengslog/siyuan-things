...
| **项目与分类** | 通过项目、区域、可排序与折叠的标题分组和标签组织不同层级的工作，活动任务与已完成任务分区展示。 |
...
3. 在项目和标签详情中点击“+”时，新建卡片会出现在当前阅读位置；项目分组可拖动排序和折叠，“已完成”固定在底部。
4. 在任务卡片中设置项目、区域、标签、日期、截止日期、提醒、重复规则、备注与检查项；需要分享时可一键复制为 Markdown。
5. 在 AI 面板中用自然语言创建、查询、修改或删除任务；需要指定归属时，可将侧边栏项目、区域、标签或日期视图拖入输入框。
6. 在思源插件菜单中点击“Things 任务管理”可直接打开设置；任务管理页面仍从左侧 Things Dock 进入。

## AI 配置与隐私

插件可以复用思源中已经配置的 AI 服务（模型列表按服务商分组，默认跟随思源 AI 当前选择的模型），也可以在插件设置中填写自定义的 OpenAI 兼容接口地址、API Key 和模型名称。
