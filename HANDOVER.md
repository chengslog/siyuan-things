...
## 📍 当前进度快照（2026-08-27：0.2.16）

- **AI 模型与服务商解析**：模型下拉按服务商分组；选中优先对齐思源 `ai.agent.modelId`，请求实际使用该模型所属服务商的 `baseURL/apiKey`（解析逻辑在 `src/utils/aiConfig.ts`，含单测），不再假定 `providers[0]`；每次发送先读内存配置、再经 `/api/system/getConf` 刷新拿最新值。
- **AI 配置热更新**：设置页改动 AI 模式或自定义接口、密钥、模型（含重置设置）后派发 `things-ai-config-change`，App 立即重读 `aiConfig`；所选模型不在可用列表时自动回落到第一个可用模型。
- **AI 流式滚动跟随**：`AIChatCore` 用 ResizeObserver + MutationObserver 驱动输出期间自动跟底；用户上滚超过 56px 暂停跟随，滚回底部附近恢复；发送前后的滚动改用 `auto`，避免平滑动画期间覆盖用户滚动。
- **澄清操作区**：创建/查询快捷按钮移出回答气泡，改为气泡下方带“你希望我怎么处理？”提示的低强调胶囊操作区；确认后携带原问题与明确意图重新发送。
- **区域详情三段式**：区域视图拆为备注（`AreaPanel showNotes`）→ 直属任务（“任务”标题、独立空态、新建落点在任务区段）→ 项目列表（`AreaPanel showProjects`，移至底部）；区域任务过滤改为 `areaId === viewId && !projectId`，不再包含项目内任务。
- **已完成分组**：项目/区域/标签视图将已完成任务集中到 `__completed__` 底部分组并按完成时间倒序；已完成卡片收缩态行首显示完成日期（原先仅日志视图）；该分组不参与拖拽排序和“+”拖放落点。
- **备注图片预览**：任务备注中的图片点击进入全屏 `dialog` 预览（遮罩或 × 关闭），点击备注其余区域仍进入编辑。
- **页签复用修复**：自定义页签 init 窗口期（应用已挂载、`thingsTab` 引用未就绪）视为可复用，修复区域内点击项目重复创建页签；`things-navigate` 监听抽为具名方法并在 `onunload` 移除。
- **设置页分区重构**：设置页统一 `things-settings__section` 分区样式，AI 设置为开关 + 左竖线缩进的层级视觉，显隐改用 `hidden` 属性；说明文字颜色改主题变量修复暗色主题。侧边栏项目/区域/标签行移除悬停星芒把手，整行即 AI 拖拽来源（`dataset.aiContext`）。
...
- **侧边栏 → AI 拖拽上下文**：主导航整行可拖；项目/区域/标签整行统一承载列表排序与 AI 拖拽（拖入 AI 输入框即添加上下文，列表内拖动仍用于排序）。数据通过 `application/x-siyuan-things-context` 传递，输入区显示带来源图标的小胶囊。
