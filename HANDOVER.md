...
## 📍 当前进度快照（2026-08-21：0.2.10）

- **侧边栏 → AI 拖拽上下文**：主导航整行可拖；项目/区域/标签为避免与原有行排序冲突，仅名称文字使用原生拖拽。数据通过 `application/x-siyuan-things-context` 传递，输入区显示带图标的白色小胶囊。
...
- **界面修复**：修复非任务回答重复与错位，顶部入口改用 Things Logo，并优化 AI 输入卡片和设置危险操作区域。
- **测试与构建**：`pnpm test` 现有 5 项核心测试通过，`pnpm run build` 通过。
